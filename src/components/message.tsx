...
export function Message({ id: messageId, text, amountOfReactions, answered = false }: MessageProps) {
    ...
    return (
        <li data-answered={answered} className="ml-4 leading-relaxed text-zinc-100 data-[answered=true]:opacity-50 data-[answered=true]:pointer-events-none">
            ...
            {hasReacted ? (
                <button onClick={removeMessageReactAction} type="button" className="mt-3 flex items-center gap-2 text-orange-400 text-sm font-medium hover:text-orange-500">
                    <ArrowUp className="size-4" />
                    Curtir pergunta({amountOfReactions})
                </button>
            ) : (
                <button type="button" onClick={createMessageReactAction} className="mt-3 flex items-center gap-2 text-zinc-300 text-sm font-medium hover:text-zinc-400">
                    <ArrowUp className="size-4" />
                    Curtir pergunta({amountOfReactions})
                </button>
            )}
